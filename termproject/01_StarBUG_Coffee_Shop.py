...
class CupOfCoffee:
    ...
    def get_price(self, coffee, drinking):
        check = 0
        if drinking.lower() == 'h' : num_price = 0; self.drinking_return = 'hot'
        elif drinking.lower() == 'c' : num_price = 1; self.drinking_return = 'cold'
        else: num_price = 2; self.drinking_return = 'frappe'
    ...
    def cal_total_price(self, select_coffee, select_drinking, add_on_input):
        ...
        if select_drinking.lower() == 'h' : num_price = 0; main_manu.append('hot')
        elif select_drinking.lower() == 'c' : num_price = 1; main_manu.append('cold')
        else: num_price = 2; main_manu.append('frappe')
        for key, value in self.price.items():
            if int(select_coffee) - 1 == int(key):
                main_manu.append(value[num_price])
        if add_on_input != "":
            for item in add_on_input:
                topping[self.get_AddOn[int(item) -1] ] = self.get_AddOnPrice[int(item) -1]
        else:
            add_on_input = []
...
class CustomerBill:
    ...
    def receipt(self):
        ...
        print(f"{' ' * int(((32 - len(len_t)) // 2))}Kun {self.name}'s Receipt{' ' * int(((32 - len(len_t)) // 2))}")
        print('++++++++++++++++++++++++++++++++')
...
def runStarBUGcafe_main():
    ...
    while name != "Good Day":
        ...
        for i in range(int(num_order)):
            ...
            while not select_coffee.isdigit() or int(select_coffee) <= 0 or int(select_coffee) > len(coffee_type):
                print(' ERROR: Invalid input!')
                select_coffee = input(f"Cup #{i + 1}, please select type of coffee: ")
            ...
            if len(cc) != 1:
                ...
                while select_drinking.lower() not in drink_type_letters or select_drinking.upper() not in cc:
                    print(' ERROR: Invalid input!')
                    select_drinking = input(ff)
            else:
            ...
            if int(check_add_on) != 0:
                while True:
                    add_on_input = input(f"Cup #{i + 1}, please select add on (enter for exit): ")
                    if add_on_input != '':
                        if not add_on_input.isdigit() or int(add_on_input) > range_add_on or int(
                                add_on_input) <= 0 or add_on_input in add_on_num:
                            print(' ERROR: Invalid input!')
                        else:
